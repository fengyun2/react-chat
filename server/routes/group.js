...
async function getGroupOnlineMembers(group) {
  const sockets = await Socket.find({ user: group.members }, { os: 1, browser: 1, environment: 1, user: 1 }).populate(
    'user',
    { username: 1, avatar: 1 },
  );
...
module.exports = {
  ...
  async leaveGroup(ctx) {
    ...
    const group = await Group.findOne({ _id: groupId });
    assert(groupId, '群组不存在');

    // 默认群组没有creator
    if (group.creator) {
      assert(group.members.toString() !== ctx.socket.user.toString(), '群主不可以退出自己创建的群');
    }
