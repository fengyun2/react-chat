...
import { Circle } from '@/components/basic/Progress';
import Dialog from '@/components/basic/Dialog';
import MessageBox from '@/components/basic/Message';
...
class Message extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return !(
      ...
      this.state.showCode === nextProps.showCode &&
      this.state.showImage === nextProps.showImage
    );
  }
  showCode = () => {
    this.setState({
  ...
  joinGroup = async () => {
    ...
    if (!err) {
      ...
      const [err2, messages] = await fetch('getLinkmanHistoryMessage', {
        linkmanId: res._id,
        existCount: 0,
      });
      if (!err2) {
  ...
  renderText() {
    ...
    return <div className="text" dangerouslySetInnerHTML={{ __html: Message.convertExpression(content) }} />;
  }
  renderImage() {
  ...
  renderContent() {
    ...
    switch (type) {
      ...
      default: {
        return (
          <div className="unknow">不支持的消息类型</div>
        );
      }
    }
  }
  ...
  render() {
    const { avatar, nickname, time, type, isSelf, tag } = this.props;
    return (
