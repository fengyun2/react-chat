  ...
  renderMessage(message) {
    ...
    const props = {
      ...
      shouldScroll:
        ...
        this.$list.scrollTop > this.$list.scrollHeight - this.$list.clientHeight * 2,
    };
    if (props.type === 'image') {
...
export default connect((state) => {
  ...
  if (!isLogin) {
    const defaultGroupMessages = state.get(['user', 'linkmans', 0, 'messages']);
    return {
      self: '',
