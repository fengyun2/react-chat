...
function reducer(state = initialState, action) {
  switch (action.type) {
    case 'Logout': {
      const keepKeys = [['ui', 'primaryColor'], ['ui', 'primaryTextColor'], ['ui', 'backgroundImage'], ['ui', 'sound']];
    ...
    case 'SetLinkmanMessages': {
      const newLinkmans = state
        ...
        .sort((linkman1, linkman2) => {
          ...
          const time1 =
            messages1.size > 0 ? messages1.get(messages1.size - 1).get('createTime') : linkman1.get('createTime');
          const time2 =
            messages2.size > 0 ? messages2.get(messages2.size - 1).get('createTime') : linkman2.get('createTime');
    ...
    case 'SetGroupMembers': {
      const linkmanIndex = state.getIn(['user', 'linkman']).findIndex(l => l.get('_id') === action.groupId);
      return state.setIn(['user', 'linkman', linkmanIndex, 'members'], immutable.fromJS(action.members));
    }
    case 'SetGroupAvatar': {
      const linkmanIndex = state.getIn(['user', 'linkmans']).findIndex(l => l.get('_id') === action.groupId);
    ...
    case 'SetFocus': {
      ...
      return state.set('focus', action.linkmanId).setIn(['user', 'linkmans', linkmanIndex, 'unread'], 0);
    }
    case 'SetFriend': {
      const linkmanIndex = state.getIn(['user', 'linkmans']).findIndex(l => l.get('_id') === action.linkmanId);
